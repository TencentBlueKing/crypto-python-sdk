...
class TestSM2LegacyVectors:
    ...
    @pytest.mark.parametrize(
        "name", ["text.sm2.nodejs.encrypt", "text.sm2.python.encrypt"]
    )
...
class TestSM2Roundtrip:
    @classmethod
    def test_load_public_key_from_pkey__rejects_invalid_key(cls) -> None:
        with pytest.raises(TypeError, match="Expected an SM2 private key"):
            SM2AsymmetricCipher.load_public_key_from_pkey(object())
    ...
    def test_encrypt__roundtrips_with_regenerated_key(cls) -> None:
        ...
        assert (
            cipher.decrypt(cipher.encrypt("emoji 😄 中文 English"))
            == "emoji 😄 中文 English"
        )
